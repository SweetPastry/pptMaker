...
\begin{document}
	...
	% 13.
	\begin{question}
		已知函数 $f(x) = x^3 (a \cdot 2^x - 2^{-x})$ 是偶函数，则 $a = $ \rule{2cm}{0.4pt} 。
	\end{question}
	...
	\begin{question}
		...
		若 $|FQ| = 6$，则 $C$ 的准线方程为 \rule{2cm}{0.4pt} 。
	\end{question}
	
	% 15.
	\begin{question}
		函数 $f(x) = |2x - 1| - 2 \ln x$ 的最小值为 \rule{2cm}{0.4pt} 。
	\end{question}
	...
	\begin{question}
		...
		以此类推。则对折 $4$ 次共可以得到不同规格图形的种数为 \rule{2cm}{0.4pt} ；
		如果对折 $n$ 次，那么 $\sum_{k=1}^n S_k = $ \rule{2cm}{0.4pt} \unit{dm^2}。
	\end{question}
